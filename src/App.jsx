        ...
        <div className="max-w-screen-2xl mx-auto px-6 md:px-12 relative z-10 grid lg:grid-cols-2 gap-16 items-center py-20 w-full">
          <div className="space-y-8">
            ...
            <p className="text-xl md:text-2xl font-medium leading-relaxed max-w-xl" style={{color:"rgba(197,202,233,0.9)"}}>
              Personalized AI analysis और 2500+ questions specifically designed for the brave aspirants of Devbhoomi.
            </p>
        ...
        <div className="max-w-screen-2xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
            ...
            <div className="md:col-span-4 bg-white rounded-3xl p-10 flex flex-col justify-center shadow-xl border-b-8 hover:-translate-y-2 transition-all" style={{borderBottomColor:"#E65100"}}>
              <span style={{fontSize:"48px", marginBottom:"24px", display:"block"}}>📚</span>
              <h3 className="text-3xl font-black mb-4 font-headline" style={{color:"#0D1B3E"}}>2500+ अभ्यास प्रश्न</h3>
              <p className="text-lg font-medium" style={{color:"#616161"}}>Extensive question bank covering Uttarakhand History, Geography, and current affairs.</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch">
            ...
            <div className="p-10 rounded-3xl flex flex-col relative overflow-hidden border-4 shadow-2xl scale-105 aipan-pattern-card" style={{background:"#0D1B3E", borderColor:"rgba(244,123,32,0.3)"}}>
              ...
              <div className="mb-8">
                ...
                <div className="text-5xl font-black mt-3 text-white">₹299 <span className="text-lg font-bold" style={{color:"rgba(197,202,233,0.7)"}}>/month</span></div>
              </div>
              <ul className="space-y-4 mb-10 flex-grow">
            ...
            <div className="p-10 rounded-3xl flex flex-col border hover:shadow-xl transition-all" style={{background:"#F9F9F9", borderColor:"#EEEEEE"}}>
              <div className="mb-8">
                ...
                <div className="text-5xl font-black mt-3" style={{color:"#0D1B3E"}}>₹1,999 <span className="text-lg font-bold" style={{color:"#9E9E9E"}}>one-time</span></div>
              </div>
              <ul className="space-y-4 mb-10 flex-grow">
...
function PracticePage() {
  ...
  const loadData = async () => {
    ...
    const allSets = [...SAMPLE_SETS, ...(setsData || [])].map(s => ({
      ...s,
...
function QuizPage({ setId }) {
  ...
  const loadQuiz = async () => {
    setLoadingQ(true);
    let foundSet = SAMPLE_SETS.find(s => s.id === setId);
    if (!foundSet) {
      const { data } = await supabase.getAll("practice_sets");
      foundSet = (data || []).find(s => s.id === setId);
    }
    ...
    if (sqResult && sqResult.length > 0) {
      ...
      qs = foundSet.question_ids.map(qid => 
        (allDbQ||[]).find(q => q.id === qid) || SAMPLE_QUESTIONS.find(q => q.id === qid)
      ).filter(Boolean);
    }
    setQuestions(qs);
    ...
    try {
      ...
      if (qs.length === 0) {
        ...
        if (Array.isArray(allData) && allData.length > 0) {
          ...
          // Last fallback — sample questions
          const d = new Date().getDate();
          qs = Array.from({length:5}, function(_,i){ return SAMPLE_QUESTIONS[(d+i) % SAMPLE_QUESTIONS.length]; });
        }
      }
      ...
      const d = new Date().getDate();
      setQuestions(Array.from({length:5}, function(_,i){ return SAMPLE_QUESTIONS[(d+i) % SAMPLE_QUESTIONS.length]; }));
    }
    setLoadingQ(false);
  };
...
function AdminDaily() {
  ...
  const loadData = async () => {
    ...
    setAllQ([...SAMPLE_QUESTIONS, ...(qRes.data||[])]);
